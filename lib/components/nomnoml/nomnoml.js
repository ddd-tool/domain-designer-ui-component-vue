import { graphlib, layout as layout$1 } from 'graphre'
...
function indexBy(list, key) {
  const obj = {}
  for (let i = 0; i < list.length; i++) obj[list[i][key]] = list[i]
  return obj
}
...
function drawTerminators(g, config, r) {
  ...
  function drawArrow(nv, size, isOpen, end) {
    ...
    const arrow = [add(x(10), y(4)), x(isOpen && !config.fillArrows ? 5 : 10), add(x(10), y(-4)), end]
...
function GraphicsCanvas(canvas) {
  ...
  function tracePath(path, offset, s) {
    ...
    for (let i = 1, len = path.length; i < len; i++) ctx.lineTo(offset.x + s * path[i].x, offset.y + s * path[i].y)
    return chainable
  }
  return {
    mousePos: function () {
...
function GraphicsSvg(document) {
  ...
  const measurementCanvas = document ? document.createElement('canvas') : null
  ...
  class Element {
    ...
    serialize() {
      ...
      else if (this.children.length === 0) return this.elideEmpty ? '' : `<${this.name} ${attrs}></${this.name}>`
      else
  ...
  function findCircleAndIntersections(A, B, C, radius) {
  ...
  function determineAngleDirection(A, B, C) {
    const ABx = B.x - A.x
    const ABy = B.y - A.y
    const BCx = C.x - B.x
    const BCy = C.y - B.y
  ...
  function arePointsCollinear(A, B, C, m) {
    const crossProduct = (B.x - A.x) * (C.y - A.y) - (B.y - A.y) * (C.x - A.x)
    return Math.abs(crossProduct) <= m
  }
  return {
    width: function () {
      return 0
    ...
    arcTo: function (x1, y1, x2, y2, radius) {
      ...
      if (!arePointsCollinear({ x: x1, y: y1 }, { x: x2, y: y2 }, previousPoint, 0.01) && radius > 0) {
        const {
          angleDirection,
          tangentPoints: [p1, p2],
        } = findCircleAndIntersections(previousPoint, { x: x1, y: y1 }, { x: x2, y: y2 }, radius)
        last(current.children).attr.d += [
          `L ${p1.x} ${p1.y}`,
          `A ${radius * 2} ${radius * 2} 0 0 ${angleDirection === 'Clockwise' ? 0 : 1} ${p2.x} ${p2.y}`,
          ``,
        ].join(' ')
        previousPoint = { x: p2.x, y: p2.y }
        return
      }
    ...
    lineTo: function (x, y) {
      if (inPathBuilderMode) last(current.children).attr.d += 'L' + x.toFixed(1) + ' ' + y.toFixed(1) + ' '
      ...
      if (ctx) {
        if (current)
          ctx.font = `${getDefined(current, (e) => e.attr['font-weight'])} ${getDefined(
            current,
            (e) => e.attr['font-style']
          )} ${getDefined(current, (e) => e.attr['font-size'])} ${getDefined(current, (e) => e.attr['font-family'])}`
        else
